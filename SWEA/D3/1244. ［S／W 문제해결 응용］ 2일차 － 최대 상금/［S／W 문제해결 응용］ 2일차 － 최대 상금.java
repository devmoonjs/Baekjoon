...
class Solution
{
    ...
    static private void dfs(int count) {
        StringBuilder targetNum = new StringBuilder();
        for (String a : arr) {
            targetNum.append(a);
        }
        
        String currentNum = targetNum.toString();
        if (visited.contains(currentNum + count)) {
            return;
        }
        visited.add(currentNum + count);

        if (count == chance) {
            int cNum = Integer.parseInt(currentNum);
            answer = Math.max(answer, cNum);
            return;
        }
